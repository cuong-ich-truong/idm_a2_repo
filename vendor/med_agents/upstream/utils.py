...
def fully_decode(
    ...
    if args.method == "base_direct":
        ...
        output = handler.get_output_multiagent(
            user_input=direct_prompt, temperature=0, max_tokens=50, system_role=""
        )
        ans, output = cleansing_final_output(output)
    elif args.method == "base_cot":
        cot_prompt = get_cot_prompt(question, options)
        output = handler.get_output_multiagent(
            user_input=cot_prompt, temperature=0, max_tokens=300, system_role=""
        )
        ans, output = cleansing_final_output(output)
    else:
        ...
        option_analyses = cleansing_analysis(
            tmp_option_analysis, options_domains, "option"
        )

        if args.method == "anal_only":
            answer_prompt = get_final_answer_prompt_analonly_with_evidence(
                question,
                options,
                question_analyses,
                option_analyses,
            ...
            output = handler.get_output_multiagent(
                user_input=answer_prompt, temperature=0, max_tokens=2500, system_role=""
            )
            ans, output = cleansing_final_output(output)
        else:
            # get synthesized report
            q_analyses_text = transform_dict2text(
                question_analyses, "question", question
            )
            o_analyses_text = transform_dict2text(option_analyses, "options", options)
            synthesizer, prompt_get_synthesized_report = (
                get_synthesized_report_prompt_with_evidence(
                    q_analyses_text, o_analyses_text, evidence_context
                )
            )
            ...
            if args.method == "syn_only":
                ...
                answer_prompt = get_final_answer_prompt_wsyn_with_evidence(
                    syn_report, evidence_context
                )
                output = handler.get_output_multiagent(
                ...
                while num_try < args.max_attempt_vote and hasno_flag:
                    ...
                    for domain in all_domains:
                        ...
                        domain_opinion = cleansing_voting(
                            raw_domain_opi
                        )  # "yes" / "no"
                    ...
                    if hasno_flag:
                        ...
                        syn_report = cleansing_syn_report(
