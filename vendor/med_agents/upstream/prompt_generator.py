...
def get_question_domains_prompt(question):
    question_domain_format = "Medical Field: " + " | ".join(
        ["Field" + str(i) for i in range(NUM_QD)]
    )
...
def get_question_analysis_prompt_with_evidence(
    ...
    """
    Backwards-compatible helper: evidence-aware variant used by our POC.
    """
...
def get_options_domains_prompt(question, options):
    options_domain_format = "Medical Field: " + " | ".join(
        ["Field" + str(i) for i in range(NUM_OD)]
    )
...
def get_options_analysis_prompt(question, options, op_domain, question_analysis):
    ...
    for _domain, _analysis in question_analysis.items():
        prompt_get_options_analyses += (
            f"The evaluation from the {_domain} expert suggests: {_analysis} \n"
        )
        prompt_get_options_analyses += (
...
def get_final_answer_prompt_analonly(
    ...
    prompt += (
        f"Step 2: Evaluate each presented option individually, based on both the specifics of the patient's scenario as well as your medical knowledge. "
    ...
    prompt += (
        f"Step 3: Based on the understanding gathered from the above steps, select the optimal choice to answer the question. \n"
        f"Points to note: \n"
...
def get_final_answer_prompt_wsyn(syn_report):
    prompt = (
        f"Here is a synthesized report: {syn_report} \n"
...
def get_final_answer_prompt_analonly_with_evidence(
    question, options, question_analyses, option_analyses, evidence_context
):
    prompt = get_final_answer_prompt_analonly(
        question, options, question_analyses, option_analyses
    )
    if evidence_context:
        prompt += (
            "\n\nOptional evidence excerpts (cite as [E1] if you use them):\n"
            f"{evidence_context}\n"
        )
    return prompt


def get_final_answer_prompt_wsyn_with_evidence(syn_report, evidence_context):
    prompt = get_final_answer_prompt_wsyn(syn_report)
    if evidence_context:
        prompt += (
            "\n\nOptional evidence excerpts (cite as [E1] if you use them):\n"
            f"{evidence_context}\n"
...
def get_cot_prompt(question, options):
    cot_format = (
        f"Thought: [the step-by-step thoughts] \n"
...
def get_synthesized_report_prompt(question_analyses, option_analyses):
    ...
    syn_report_format = (
        f"Key Knowledge: [extracted key knowledge] \n"
...
def get_synthesized_report_prompt_with_evidence(
    question_analyses, option_analyses, evidence_context
):
    synthesizer, prompt = get_synthesized_report_prompt(
...
def get_consensus_prompt(domain, syn_report):
    ...
    cons_prompt = (
        f"Here is a medical report: {syn_report} \n"
...
def get_revision_prompt(syn_report, revision_advice):
    ...
    for domain, advice in revision_advice.items():
        revision_prompt += (
            f"Here is advice from a medical expert specialized in {domain}: {advice}.\n"
        )
